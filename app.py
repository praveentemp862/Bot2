...
def calculate_crypto_fee(amount):
    ...
    if amount < 10:
        ...
        return amount * 0.002  # 0.2% fee (fixed: was 0.2 which is 20%)
...
def calculate_inr_fee(amount):
    ...
    if amount < 100:
        ...
        return amount * 0.05  # 5% fee for amounts 100 and above (fixed: was 0.1 which is 10%)
...
def main():
    ...
    # Start bot - FIXED: Use proper asyncio event loop handling
    print("Bot is running...")
    print("Make sure mm.jpg file exists in the same directory!")
    
    # Fix for the polling cleanup error
    try:
        # Run the bot with proper error handling
        application.run_polling(allowed_updates=Update.ALL_TYPES)
    except AttributeError as e:
        if "'Updater' object has no attribute '_Updater__polling_cleanup_cb'" in str(e):
            print("Detected known python-telegram-bot error. Attempting to restart...")
            # Alternative approach - restart with different method
            import asyncio
            try:
                ...
                print("Try updating python-telegram-bot: pip install --upgrade python-telegram-bot")
                sys.exit(1)
        else:
            raise
